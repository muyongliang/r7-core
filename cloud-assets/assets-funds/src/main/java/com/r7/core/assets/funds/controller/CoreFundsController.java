...
public class CoreFundsController {
    ...
    public ResponseEntity saveFunds(@Valid @RequestBody CoreFundsDTO coreFundsDto) {
        return ResponseEntity.success(coreFundsService.saveFunds(coreFundsDto, RequestHolder.getAppId(), RequestHolder.getOrganId(), RequestHolder.getUserId()));
    }
    ...
    public ResponseEntity pageFundsByUserId(@PathVariable(value = "userId") Long userId,
                                            @RequestParam(value = "status", required = false) FundsStatusEnum status,
                                            @RequestParam(value = "transactionStatus", required = false) FundsTransactionStatusEnum transactionStatus,
                                            @RequestParam(value = "channel", required = false) FundsChannelEnum channel,
    ...
    @GetMapping("/page")
    public ResponseEntity pageFundsByAppId(@RequestParam(value = "status", required = false) FundsStatusEnum status,
                                           @RequestParam(value = "transactionStatus", required = false) FundsTransactionStatusEnum transactionStatus,
                                           @RequestParam(value = "channel", required = false) FundsChannelEnum channel,
                                           @RequestParam(value = "startDate", required = false) String startDate,
                                           @RequestParam(value = "endDate", required = false) String endDate,
                                           @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                           @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize) {
        return ResponseEntity.success(coreFundsService.pageFundsByAppId(RequestHolder.getAppId(), status, transactionStatus, channel, startDate, endDate, pageNum, pageSize));
    }
