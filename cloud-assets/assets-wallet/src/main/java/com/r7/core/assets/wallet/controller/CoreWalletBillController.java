...
public class CoreWalletBillController {
    ...
    public ResponseEntity pageWalletBillByUserId(@PathVariable(value = "userId") Long userId,
        ...
        return ResponseEntity.success(coreWalletBillService.pageWalletBillByUserId(userId, type, source, status, startDate, endDate, pageNum, pageSize));
    }


    @ApiOperation(value = "根据平台id和交易来源分页展示钱包账单", response = CoreWalletBillPageVO.class)
    @GetMapping("/page")
    public ResponseEntity pageAppIdWalletBillByType(@RequestParam(value = "type", required = false) Integer type,
                                                    @RequestParam(value = "source", required = false) String source,
                                                    @RequestParam(value = "status", required = false) Integer status,
                                                    @RequestParam(value = "startDate", required = false) String startDate,
                                                    @RequestParam(value = "endDate", required = false) String endDate,
                                                     @RequestParam(defaultValue = "1") Integer pageNum,
                                                     @RequestParam(defaultValue = "10") Integer pageSize) {
        return ResponseEntity.success(coreWalletBillService.pageWalletBillByAppId(2L, type, source, status, startDate, endDate, pageNum, pageSize));
    }
    ...
    public ResponseEntity getTotalConsumptionBalance(@RequestParam(value = "startDate", required = false) String startDate,
                                                     @RequestParam(value = "endDate", required = false) String endDate) {
        return ResponseEntity.success(coreWalletBillService.getTotalConsumptionBalance(2L, startDate, endDate));
    }
}
